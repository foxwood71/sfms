...
- `.mise.toml` 예: `node = "22"`, `python = "3.12"`, `pnpm = "9"`
- 새 터미널에서도 자동 버전 전환
...
- `.env` 자동 로드: `python-dotenv` deps 확인, `load_dotenv()` in main.py
- DB: PostgreSQL 로컬 실행 후 `DATABASE_URL` 설정
...
### 6.4 (로컬 개발/Podman) 추천 방식

로컬 1세트(개발용)라면 시스템 경로에 넣기보다, 프로젝트 외부의 “secrets 디렉터리”에 두고 **Podman secret로 주입**하는 게 관리가 편합니다. Podman secret는 컨테이너 안에서 `/run/secrets/<secretname>` 파일로 접근하는 방식이 기본입니다
...
`podman secret create`는 파일 경로를 입력으로 받아 secret을 만들 수 있습니다

## 6.5 리버스 프록시 컨테이너에 TLS 적용 (NGINX / Traefik)

리버스 프록시 컨테이너에서 TLS 종료를 할 때는 “컨테이너 내부에서 읽을 수 있는 경로에 인증서/키가 있어야” 하고, NGINX는 `ssl_certificate`, `ssl_certificate_key`로 파일 경로를 지정합니다. Podman을 쓰면 TLS 키/인증서를 `podman secret`으로 넣고 기본 경로(`/run/secrets/...`)로 읽게 만들 수 있습니다

### 6.5.1 공통 전제 (파일/시크릿 이름)

- 인증서: `server.crt` (또는 fullchain 성격이면 `server.crt`에 체인 포함)
- 개인키: `server.key` (외부 노출 금지)
- Podman secret 이름 예시: `tls_crt`, `tls_key` (컨테이너 내부 기본 경로는 `/run/secrets/tls_crt`, `/run/secrets/tls_key`)
...
Traefik은 파일 provider(dynamic configuration)로 `certFile`, `keyFile` 경로를 지정해 인증서를 로드할 수 있습니다.
...
Traefik 문서에서도 이 방식은 file provider로 정의하는 형태를 예시로 들고 있습니다

### 6.5.4 Podman secret로 컨테이너에 주입하는 방법 (개념)

- secret 생성: `podman secret create <이름> <파일>` 형태로 만들 수 있습니다
- 컨테이너 실행 시 `--secret` 옵션으로 주입하며, 기본 마운트 위치는 `/run/secrets/<secretname>`이고 `target=`으로 파일명/경로를 바꿀 수 있습니다
...
| 증상 | 해결 |
| ------ | ------ |
| `mise: command not found` | `mise activate bash` → `source ~/.bashrc` |
| 버전 안 바뀜 | `mise trust --force` 재실행 |
| uv sync 느림 | `uv cache clean` 후 재시도 |
| .env 안 로드 | `uv add python-dotenv` + `load_dotenv()` |

셋업 완료! `mise doctor`로 환경 진단 후 개발 시작하세요.
