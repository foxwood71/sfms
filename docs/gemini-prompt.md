...
- **Framework**: FastAPI (full async/await 패턴 필수)
- **Database**: PostgreSQL 16+
- **ORM**: SQLAlchemy (Async Engine + asyncpg driver, 동기 코드 금지)
...
graph TD
    ...
    subgraph DockerNet ["🐳 Docker(sfms net)"]
        ...
        subgraph AppZone ["Application Services"]
            ...
            Backend -->|SQL| DB[("PostgreSQL Async DB queries via SQLAlchemy asyncpg")]
...
│   │   └── database.py        #  DB 세션 관리 (app.core.database, AsyncSessionLocal 정의)
│   │
...
4. DB Connection: DB 연결 객체 명칭은 app.core.database로 통일 및 async 구현 (예: async def get_db() -> AsyncSession)

5. Router/Service: 모든 DB 작업 async def로 작성. 동시성 최적화 위해 blocking 코드 피함.
...
1. Phase 1 (환경 구축): Docker Compose (DB, MinIO, FastAPI async endpoints, React) 실행 확인,  .
