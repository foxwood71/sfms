...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="소프트 삭제된 파일을 영구 파기합니다."
    )
    parser.add_argument(
        "--days", type=int, default=30, help="삭제 유지 기간 (기본 30일)"
    )
    ...
    async with async_session_factory() as db:
        try:
            deleted_count = await AttachmentService.purge_deleted_attachments(
                db, days_threshold=args.days
            )
