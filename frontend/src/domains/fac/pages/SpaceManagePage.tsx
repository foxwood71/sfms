...
import { getFacilitiesApi, getSpaceTreeApi, updateSpaceApi, createSpaceApi, deleteSpaceApi } from "../api";
...
const SpaceManagePage: React.FC = () => {
	...
	// 선택된 시설의 공간 트리 조회
	const { data: spaceTree, isFetching: isTreeLoading } = useQuery({
		queryKey: ["spaces", "tree", selectedFacilityId],
		queryFn: () => selectedFacilityId ? getSpaceTreeApi(selectedFacilityId) : Promise.resolve({ data: [] }),
	...
	const saveMutation = useMutation({
		mutationFn: (values: any) => {
			if (editingSpace) return updateSpaceApi(editingSpace.id, values);
	...
	const initialSplitterSize = useMemo(() => {
		...
		return saved && !isNaN(Number(saved)) ? Number(saved) : "30%";
	}, []);
	...
	const treeData = useMemo(() => {
		const mapNodes = (items: SpaceType[]): any[] => {
			return items
				.map((item) => {
		...
		const rootTitle = facilities?.data?.find(f => f.id === selectedFacilityId)?.name || t("fac.space.tree_title");
		...
		return [
			{
				...
				children: mapNodes(spaceTree?.data || []),
			}
		];
	}, [spaceTree, facilities, selectedFacilityId, searchValue, token, t]);
	...
	return (
				...
				extra: [
					facilities?.data && facilities.data.length > 0 ? (
						<Select
							...
							style={{ width: 220 }}
							options={facilities.data.map(f => ({ label: f.name, value: f.id }))}
							onChange={(val) => {
		...
		>
			...
			<Splitter style={{ height: "100%", background: token.colorBgContainer }} onResizeEnd={handleSplitterChange}>
							...
							<Space size={2}>
								...
								<Tooltip title={t("common.reload")}>
									<Button
										type="text"
										icon={<ReloadOutlined />}
										onClick={() => queryClient.invalidateQueries({ queryKey: ["spaces", "tree", selectedFacilityId] })}
					...
					>
						{selectedFacilityId ? (
							<div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
								{showFilter && (
									<div style={{ padding: "12px", background: token.colorFillAlter, marginBottom: 12, borderRadius: token.borderRadiusLG, margin: "0 16px 8px 16px" }}>
										<Input.Search
											placeholder="공간명 검색..."
											size="small"
											allowClear
								...
								{selectedSpace && (
									<div style={{ padding: "12px 16px", borderTop: `1px solid ${token.colorBorderSecondary}`, background: token.colorBgLayout }}>
										<Button 
											type="dashed" 
											block 
											icon={<PlusOutlined />} 
											onClick={() => {
												setEditingSpace({ parent_id: selectedSpace.id } as any);
												setDrawerOpen(true);
											}}
										>
								...
								<Empty 
									description={
										<Space direction="vertical" align="center">
											...
											{facilities?.data?.length === 0 && (
												<Button type="primary" size="large" onClick={() => navigate("/fac/facilities")} style={{ marginTop: 12 }}>
													{t("fac.facility.new_facility") || "신규 시설 등록하러 가기"}
												</Button>
											)}
										</Space>
									} 
						...
						extra={
							selectedSpace && (
								<Space>
									...
									>
										<Button danger type="text" icon={<DeleteOutlined />}>{t("common.delete")}</Button>
									</Popconfirm>
									<Button 
										type="primary" 
										icon={<EditOutlined />} 
							...
							<div style={{ padding: "24px", overflowY: "auto", flex: 1 }}>
								<div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
									...
									<div>
										...
										<code style={{ fontSize: 12, color: token.colorTextSecondary }}>{selectedSpace.code}</code>
									</div>
									<div style={{ marginLeft: "auto" }}>
										<Tag color={selectedSpace.is_active ? "success" : "default"}>
											{selectedSpace.is_active ? t("common.active") : t("common.inactive")}
										</Tag>
									</div>
								</div>

								<Descriptions bordered column={2} size="small">
									<Descriptions.Item label={t("fac.space.code")}>{selectedSpace.code}</Descriptions.Item>
									<Descriptions.Item label={t("fac.space.name")}>{selectedSpace.name}</Descriptions.Item>
									<Descriptions.Item label={t("fac.space.type")}>{selectedSpace.space_type_id || "-"}</Descriptions.Item>
									<Descriptions.Item label={t("fac.space.function")}>{selectedSpace.space_function_id || "-"}</Descriptions.Item>
									<Descriptions.Item label={t("fac.space.area")}>{selectedSpace.area_size?.toLocaleString()} m²</Descriptions.Item>
									<Descriptions.Item label={t("fac.facility.sort_order")}>{selectedSpace.sort_order}</Descriptions.Item>
									<Descriptions.Item label={t("fac.space.restricted")} span={2}>
										{selectedSpace.is_restricted ? <Tag color="error">RESTRICTED</Tag> : <Tag color="processing">PUBLIC</Tag>}
									</Descriptions.Item>
									<Descriptions.Item label={t("fac.space.org")} span={2}>
										{selectedSpace.org_id || t("common.none")}
									</Descriptions.Item>
									<Descriptions.Item label={t("common.description")} span={2}>
										{selectedSpace.metadata_info?.description || "-"}
									</Descriptions.Item>
								</Descriptions>

								<div style={{ marginTop: 24, padding: 16, background: token.colorInfoBg, borderRadius: 8, border: `1px dashed ${token.colorInfoBorder}` }}>
									<Space align="start">
										<InfoCircleOutlined style={{ color: token.colorInfo, marginTop: 4 }} />
										<div style={{ fontSize: 13, color: token.colorTextSecondary }}>
											{t("fac.space.hint_msg") || "이 공간은 장비(EQP) 및 자산 관리의 기본 단위가 됩니다."}
										</div>
									</Space>
								</div>
							</div>
						) : (
							<Empty description={t("common.select_placeholder")} style={{ marginTop: 100 }} />
						)}
					</ProCard>
				</Splitter.Panel>
			</Splitter>
