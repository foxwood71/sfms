...
import { createSpaceApi, deleteSpaceApi, getFacilitiesApi, getSpaceTreeApi, updateSpaceApi } from "../api";
...
const SpaceManagePage: React.FC = () => {
    ...
    const { data: spaceRes, isFetching: isTreeLoading } = useQuery({
        ...
        queryFn: async () => {
            if (!selectedFacilityId) return { data: [], success: true, domain: "FAC", code: "SUCCESS", message: "" };
    ...
    const saveMutation = useMutation({
        mutationFn: (values: SpaceParams) => {
            if (editingSpace?.id) return updateSpaceApi(editingSpace.id, values);
    ...
    const treeData = useMemo(() => {
        const mapNodes = (items: SpaceType[]): DataNode[] => {
            return items
                .map((item) => {
