import { CloseOutlined, SaveOutlined, ClusterOutlined } from "@ant-design/icons";
import { DrawerForm, ProForm, ProFormText, ProFormTextArea, ProFormDigit, ProFormSwitch } from "@ant-design/pro-components";
import { Button, Col, Row, Space, Switch, Typography, TreeSelect, theme } from "antd";
...
interface SpaceFormDrawerProps {
    ...
    facilityName: string;
    parentTreeData: any[]; // 부모 선택용 트리 데이터 추가
...
const SpaceFormDrawer: React.FC<SpaceFormDrawerProps> = ({
    ...
    return (
        <DrawerForm
            title={mode === "view" ? t("fac.space.detail_title") : mode === "edit" ? t("common.edit") : t("fac.space.new_space")}
            open={open}
            ...
            submitter={{
                render: (props) => mode === "view" 
                    ? [
                        <Button key="close" onClick={() => onOpenChange(false)}>{t("common.confirm")}</Button>,
                        <Button key="edit" type="primary" onClick={() => setMode("edit")}>{t("common.edit")}</Button>
                      ]
                    : [
                        <Button key="cancel" onClick={() => editingSpace ? setMode("view") : onOpenChange(false)}>{t("common.cancel")}</Button>,
                        <Button key="submit" type="primary" icon={<SaveOutlined />} onClick={() => props.form?.submit()}>{t("common.save")}</Button>
                      ]
            }}
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: 32, padding: "16px", background: token.colorFillAlter, borderRadius: 12 }}>
                ...
                <div>
                    <Title level={4} style={{ margin: 0 }}>{watchedName || t("fac.space.name")}</Title>
                    <Text type="secondary">{facilityName} / {form.getFieldValue("code") || "SPACE_CODE"}</Text>
                </div>
            </div>
            ...
            <Row gutter={16}>
                <Col span={12}>
                    <ProFormText name="code" label={t("fac.space.code")} disabled={mode !== "add"} rules={[{ required: true }]} fieldProps={fieldStyle} />
                </Col>
                <Col span={12}>
                    <ProFormText name="name" label={t("fac.space.name")} disabled={isReadOnly} rules={[{ required: true }]} fieldProps={fieldStyle} />
                </Col>
            </Row>
            ...
            <Row gutter={16}>
                <Col span={12}>
                    <ProFormDigit name="area_size" label={t("fac.space.area")} disabled={isReadOnly} fieldProps={fieldStyle} />
                </Col>
                <Col span={12}>
                    <ProFormDigit name="sort_order" label={t("fac.facility.sort_order")} disabled={isReadOnly} fieldProps={fieldStyle} />
                </Col>
            </Row>
            ...
            <Row gutter={16} style={{ marginTop: 8 }}>
                <Col span={12}>
                    <div style={{ padding: "8px 16px", borderRadius: 8, background: token.colorFillAlter, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <Text size="small">{t("fac.space.restricted")}</Text>
                        <ProFormSwitch name="is_restricted" noStyle disabled={isReadOnly} />
                    </div>
                </Col>
                <Col span={12}>
                    <div style={{ padding: "8px 16px", borderRadius: 8, background: token.colorFillAlter, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <Text size="small">{t("fac.facility.status")}</Text>
                        <ProFormSwitch name="is_active" noStyle disabled={isReadOnly} />
                    </div>
                </Col>
            </Row>

            <ProFormTextArea name="description" label={t("common.search_placeholder")} disabled={isReadOnly} style={{ marginTop: 16 }} fieldProps={{ autoSize: { minRows: 3 } }} />
        </DrawerForm>
    );
};
