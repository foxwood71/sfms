...
/**
 * 시스템 감사 로그 조회 페이지 (Icon-centric UI)
 */
const AuditLogPage: React.FC = () => {
	...
	const columns = [
		...
		},
		{
			...
			width: 100,
			render: (val: any) => val || t("common.none"),
		},
		{ 
		...
		>
			...
			>
				<div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "0 16px" }}>
							...
							<Row gutter={[16, 16]}>
								<Col span={6}>
									<Space direction="vertical" style={{ width: "100%" }} size={2}>
										<span style={{ fontSize: "12px", color: token.colorTextSecondary }}>{t("sys.audit.created_at")}</span>
										<RangePicker 
											size="small" 
											style={{ width: "100%" }} 
								...
								<Col span={4}>
									<Space direction="vertical" style={{ width: "100%" }} size={2}>
										<span style={{ fontSize: "12px", color: token.colorTextSecondary }}>{t("sys.audit.action_type")}</span>
										<Select
											size="small"
											style={{ width: "100%" }}
								...
								<Col span={4}>
									<Space direction="vertical" style={{ width: "100%" }} size={2}>
										<span style={{ fontSize: "12px", color: token.colorTextSecondary }}>{t("sys.audit.domain")}</span>
										<Select
											size="small"
											style={{ width: "100%" }}
											placeholder={t("common.select_placeholder")}
											...
											onChange={(val) => setFilters({ ...filters, target_domain: val })}
											options={["USR", "IAM", "FAC", "SYS", "CMM"].map(d => ({ value: d, label: d }))}
										/>
									</Space>
								</Col>
								<Col span={4}>
									<Space direction="vertical" style={{ width: "100%" }} size={2}>
										<span style={{ fontSize: "12px", color: token.colorTextSecondary }}>{t("sys.audit.actor")}</span>
										<Input
											size="small"
											placeholder="User ID"
								...
								<Col span={6}>
									<Space direction="vertical" style={{ width: "100%" }} size={2}>
										<span style={{ fontSize: "12px", color: token.colorTextSecondary }}>{t("common.search")}</span>
										<Input.Search
											size="small"
					...
					{/* [REFINE] 활성 필터 태그 영역 (Icon-centric with Tooltips) */}
					{filterTags.length > 0 && (
						<div style={{ marginBottom: 12, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
							...
							{filterTags.map(tag => (
								<Tag 
									...
									style={{ borderRadius: "12px", padding: "0 10px", background: token.colorBgContainer }}
								>
