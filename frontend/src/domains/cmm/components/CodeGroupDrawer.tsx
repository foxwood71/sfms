import { CloseOutlined, EditOutlined, SaveOutlined, FolderOutlined, SettingOutlined } from "@ant-design/icons";
import { DrawerForm, ProForm, ProFormText, ProFormTextArea, ProFormDigit, ProFormSwitch } from "@ant-design/pro-components";
import { Button, Divider, Space, theme, Switch, Typography, Col, Row, Card } from "antd";
	...
	return (
		...
		>
			...
			<div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: 32, padding: "16px", background: token.colorFillAlter, borderRadius: 12 }}>
				<div style={{ width: 48, height: 48, borderRadius: 10, background: token.colorPrimaryBg, display: "flex", justifyContent: "center", alignItems: "center" }}>
					<FolderOutlined style={{ fontSize: 24, color: token.colorPrimary }} />
				</div>
				<div>
					<Title level={4} style={{ margin: 0 }}>{watchedName || "새 그룹"}</Title>
			...
			<Row gutter={16}>
                <Col span={12}>
                    <ProFormText name="group_code" label="그룹 코드" disabled={mode !== "add"} fieldProps={fieldStyle} rules={[{ required: true }]} placeholder="영문 대문자" />
                </Col>
                <Col span={12}>
                    <ProFormText name="group_name" label="그룹 명칭" disabled={mode === "view"} fieldProps={fieldStyle} rules={[{ required: true }]} />
                </Col>
            </Row>

            <ProFormTextArea name="description" label="상세 설명" disabled={mode === "view"} fieldProps={{ ...fieldStyle, height: "auto" }} />

            <div style={{ marginTop: 24 }}>
                <Divider orientation="left" style={{ margin: '0 0 16px 0' }}>
                    <Space size={4}><SettingOutlined /> <Text strong>코드 생성 규격 설정</Text></Space>
                </Divider>
                
                {/* bordered 대신 variant="outlined" 사용 (v5 대응) */}
                <Card size="small" variant="outlined" styles={{ body: { padding: '16px' } }} style={{ background: token.colorFillQuaternary }}>
                    <Row gutter={24} align="middle">
                        ...
                        <Col span={12}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                <Text strong style={{ fontSize: '14px' }}>자산코드 순번 사용</Text>
                                <Space>
