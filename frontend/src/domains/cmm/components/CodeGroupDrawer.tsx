import { CloseOutlined, EditOutlined, FolderOutlined, SaveOutlined, SettingOutlined, AppstoreOutlined } from "@ant-design/icons";
import { DrawerForm, ProForm, ProFormDigit, ProFormText, ProFormTextArea, ProFormSelect } from "@ant-design/pro-components";
...
const CodeGroupDrawer: React.FC<CodeGroupDrawerProps> = ({
	...
	useEffect(() => {
		if (open) {
			if (editingGroup) {
				setMode("view");
				form.setFieldsValue(editingGroup as any);
			} else {
				setMode("add");
				form.resetFields();
				form.setFieldsValue({
	...
	return (
		...
		>
			...
			>
				<Text
					strong
					style={{ color: watchedActive ? token.colorSuccess : token.colorTextDisabled }}
				>
