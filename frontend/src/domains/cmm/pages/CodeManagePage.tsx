...
import { App, Button, Empty, Space, Splitter, Tooltip, theme } from "antd";
...
import type { CodeDetail, CodeGroup, CodeGroupParams, CodeDetailParams } from "../types";

/**
 * 공통 코드 관리 페이지 (Runtime & Type Error Fix)
 */
const CodeManagePage: React.FC = () => {
	...
	const groupMutation = useMutation({
		mutationFn: (values: CodeGroupParams) =>
			editingGroup
				? updateCodeGroup(editingGroup.group_code, values)
				: createCodeGroup(values),
		onSuccess: () => {
	...
	const detailColumns: ProColumns<CodeDetail>[] = [
		...
		},
		{
			...
			dataIndex: "is_active",
			width: 80,
			render: (active) => (active ? t("common.active") : t("common.inactive")),
		},
		{
	...
	return (
		...
		>
								...
								<div style={{ padding: "0 16px" }}>
									<ProTable<CodeDetail>
										...
										request={async () => {
                                            ...
                                            // [FIX] ProTable request 규격 준수: { data: [], success: true } 반환
			...
			<CodeGroupDrawer
				...
				onFinish={async (v) => {
					await groupMutation.mutateAsync(v);
					return true;
				}}
			/>
			<CodeDetailDrawer
