...
import { Button, message, Popconfirm, Space, Tag, theme, Switch, Typography } from "antd";
...
const CodeManagePage: React.FC = () => {
	...
	const filteredDetails = useMemo(() => {
		...
		const sorted = [...rawDetails].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0) || a.detail_code.localeCompare(b.detail_code));
	...
	return (
		<PageContainer header={{ title: "공통 코드 관리" }}>
			<ProCard ghost gutter={16} style={{ height: CONTENT_HEIGHT }}>
				...
				>
					<ProTable<CodeGroup>
						...
						onRow={(record) => ({
							...
							style: {
								cursor: "pointer",
								backgroundColor: selectedGroup?.group_code === record.group_code ? token.controlItemBgActive : "inherit",
							},
						})}
					/>
				</ProCard>
				...
				>
					{selectedGroup ? (
						...
						<div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center", color: token.colorTextDisabled }}>
							좌측에서 그룹을 선택해주세요.
						</div>
					)}
				</ProCard>
			...
			>
				<ProFormText name="group_code" label="그룹 코드" disabled={!!editingGroup} rules={[{ required: true }]} />
			...
			<ModalForm
				...
				modalProps={{ destroyOnClose: true }}
			>
				<ProFormText name="detail_code" label="상세 코드" disabled={!!editingDetail} rules={[{ required: true }]} />
