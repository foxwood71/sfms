...
const CodeManagePage: React.FC = () => {
	...
	const detailMutation = useMutation<unknown, Error, CodeDetailParams>({
		mutationFn: (values: CodeDetailParams) => {
	...
	return (
		...
		>
								...
								<Tree
									...
									onSelect={(keys, info) => {
										if (keys.length > 0 && info.node.isLeaf) {
											setSelectedGroup((info.node as any).data);
										}
									}}
								/>
							</div>
					...
					<Splitter.Panel>
						<ProCard
							...
							extra={
								selectedGroup && (
									<Space size={8}>
										<Button type="text" size="small" icon={<FilterOutlined style={{ color: showDetailFilter ? token.colorPrimary : undefined }} />} onClick={() => setShowDetailFilter(!showDetailFilter)} />
								...
								{showDetailFilter && (
									<div style={{ padding: "12px 20px", background: token.colorFillAlter, borderBottom: `1px solid ${token.colorBorderSecondary}` }}>
										<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
											<span style={{ fontSize: "12px", color: token.colorTextSecondary }}>{t("cmm.code.include_inactive")}</span>
								...
								{selectedGroup ? (
									<div style={{ flex: 1, overflow: "hidden", padding: "0 20px 20px 20px" }}>
										<ProTable<CodeDetail>
											...
											size={tableSize}
											rowKey="detail_code"
											rowSelection={{ selectedRowKeys: detailSelectedRowKeys, onChange: setDetailSelectedRowKeys }}
											...
											request={async () => {
												...
												const filteredData = showInactiveDetail ? details : details.filter((d: CodeDetail) => d.is_active);
												return { data: filteredData || [], success: true };
											}}
											params={{ groupCode: selectedGroup.group_code, showInactive: showInactiveDetail }}
											search={false}
											options={{ setting: true, density: false }}
											pagination={{ pageSize: detailPageSize, onChange: (_, size) => setDetailPageSize(size || 10) }}
											toolBarRender={() => [
												<Select key="dpz" size="small" value={detailPageSize} onChange={setDetailPageSize} options={[{ value: 10, label: t("user.page_size", { count: 10 }) }, { value: 20, label: t("user.page_size", { count: 20 }) }, { value: 50, label: t("user.page_size", { count: 50 }) }]} style={{ width: 80 }} />,
												<Dropdown key="dds" menu={{ items: [ { key: "large", label: t("user.density_default"), onClick: () => setTableSize("large") }, { key: "middle", label: t("user.density_middle"), onClick: () => setTableSize("middle") }, { key: "small", label: t("user.density_small"), onClick: () => setTableSize("small") } ] }} placement="bottomRight"><Tooltip title={t("user.density")}><Button type="text" icon={<LineHeightOutlined />} /></Tooltip></Dropdown>,
											]}
										/>
									</div>
								) : (
									<div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: token.colorFillQuaternary }}>
			...
			<CodeGroupDrawer open={groupDrawerOpen} onOpenChange={setGroupDrawerOpen} editingGroup={editingGroup} onFinish={async () => { 
