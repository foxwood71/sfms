...
import { App, Button, Empty, Space, Splitter, theme } from "antd";
...
const CodeManagePage: React.FC = () => {
    ...
    const groupMutation = useMutation({
        mutationFn: (values: CodeGroupParams) =>
            editingGroup ? updateCodeGroup(editingGroup.group_code, values) : createCodeGroup(values),
        onSuccess: () => {
    ...
    const detailMutation = useMutation<unknown, Error, Partial<CodeDetail>>({
        mutationFn: (values: Partial<CodeDetail>) => {
            if (!selectedGroup) {
                throw new Error("No code group selected");
            }
