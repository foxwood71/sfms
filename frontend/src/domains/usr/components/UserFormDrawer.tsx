...
import { Col, Divider, Row, theme, Space, Avatar, Upload, Button, Checkbox, message } from "antd";
import { UserOutlined, ClockCircleOutlined, EditOutlined, UploadOutlined } from "@ant-design/icons";
...
const UserFormDrawer: React.FC<UserFormDrawerProps> = ({
	...
	return (
		...
		>
			{/* 1. 프로필 이미지 영역 */}
			<div style={{ display: 'flex', alignItems: 'center', marginBottom: 24, padding: '16px', background: token.colorFillAlter, borderRadius: token.borderRadiusLG }}>
				<Space size={20}>
					<Avatar 
						size={80} 
						icon={<UserOutlined />} 
			...
			{editingUser && (
				<div style={{ marginTop: 24, padding: '16px', borderTop: `1px solid ${token.colorBorderSecondary}`, fontSize: '12px', color: token.colorTextDescription }}>
					<Row gutter={16}>
						<Col span={12}>
							<Space>
								<ClockCircleOutlined />
								<span>생성: {editingUser.created_at ? dayjs(editingUser.created_at).format("YYYY-MM-DD HH:mm") : "-"}</span>
							</Space>
						</Col>
						<Col span={12}>
							<Space>
								<EditOutlined />
								<span>수정: {editingUser.updated_at ? dayjs(editingUser.updated_at).format("YYYY-MM-DD HH:mm") : "-"}</span>
							</Space>
						</Col>
					</Row>
				</div>
