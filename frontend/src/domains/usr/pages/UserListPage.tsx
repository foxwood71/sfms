...
const UserListPage: React.FC = () => {
    ...
    const treeData = useMemo(() => {
        ...
        return [
            { key: "root", title: "전체 조직도", icon: <TeamOutlined />, children: mapToTree(orgResponse?.data || []) },
        ];
    ...
    const columns: ProColumns<User>[] = [
        ...
        { title: "이메일", dataIndex: "email", key: "email", width: 180, ellipsis: true, hideInSearch: true },
        {
            ...
            sorter: true,
            render: (dom, record) => (
                <Tag color={record.is_active ? "green" : "default"}>{record.is_active ? "재직" : "퇴사"}</Tag>
            ),
        },
        {
            title: "계정 상태",
            ...
            sorter: true,
            render: (text) => (
                <Tag color={text === "ACTIVE" ? "blue" : "error"}>{text === "ACTIVE" ? "정상" : "차단"}</Tag>
            ),
        },
        {
            title: "관리",
            ...
            hideInSetting: true,
            render: (_, record) => [
                <Tooltip key="edit-tip" title="수정">
                    <a
                        onClick={() => {
                            setEditingUser({ ...record, org_id: Number(record.org_id) });
                ...
                >
                    <Tooltip key="del-tip" title="퇴직 처리">
                        <a style={{ color: "#ff4d4f", marginLeft: 8 }}>
                            <DeleteOutlined />
                        </a>
                    </Tooltip>
                </Popconfirm>,
                            ...
                            <Space size={2}>
                                <Tooltip title="조직 검색">
                                    <Button
                                        ...
                                        icon={
                                            <SearchOutlined
                                                style={{ color: showSearch ? token.colorPrimary : undefined }}
                                            />
                                        }
                    ...
                    >
                        {showOrgFilter && (
                            <div style={{ padding: "16px 24px 8px 24px", flexShrink: 0 }}>
                                ...
                                >
                                    <span
                                        style={{ fontSize: "11px", color: token.colorTextSecondary, fontWeight: 500 }}
                                    >
                                        조직 필터
                                    </span>
                                    <Space size={4}>
                                        <span
                                            style={{
                                                fontSize: "11px",
                                                color: showInactiveOrg ? token.colorError : token.colorSuccess,
                                                fontWeight: 500,
                                            }}
                                        >
                                            {showInactiveOrg ? "비활성 포함" : "활성 부서만"}
                                        </span>
                                        <Switch
                                            size="small"
                                            checked={showInactiveOrg}
                                            onChange={setShowInactiveOrg}
                        ...
                        <div style={{ flex: 1, overflow: "auto", padding: "16px 24px" }}>
                            {isOrgLoading && !isOrgFetching ? (
                                ...
                                <Tree
                                    ...
                                    selectedKeys={selectedOrgId ? [selectedOrgId] : ["root"]}
                                    onSelect={(keys) => {
                                        if (keys.length > 0) {
                                            const key = keys[0];
                                            setSelectedOrgId(key === "root" ? undefined : Number(key));
                                        } else {
                            ...
                            <Space size={2}>
                                <Tooltip title="필터">
                                    <Button
                                        ...
                                        icon={
                                            <FilterOutlined
                                                style={{ color: showUserFilter ? token.colorPrimary : undefined }}
                                            />
                                        }
                                            ...
                                            <div style={{ display: "flex", alignItems: "center", height: "24px" }}>
                                                ...
                                                <span
                                                    style={{
                                                        marginLeft: 8,
                                                        fontSize: "12px",
                                                        color: showInactiveUser ? token.colorError : token.colorSuccess,
                                                    }}
                                                >
                                                    {showInactiveUser ? "퇴사자 포함" : "재직자만"}
                                                </span>
                                            </div>
                                        </Col>
                                    </Row>
