...
/**
 * 사용자 목록 관리 페이지 (Bento Standard v1.1)
 */
const UserListPage: React.FC = () => {
	...
	const handleBulkDelete = () => {
		...
		Modal.confirm({
			...
			content: t("common.bulk_delete_confirm", { count: selectedRowKeys.length }, `선택한 ${selectedRowKeys.length}개 항목을 정말 삭제하시겠습니까?`),
			okText: t("common.delete"),
			okType: "danger",
			cancelText: t("common.cancel"),
			onOk: async () => {
				try {
					await Promise.all(selectedRowKeys.map((id) => deleteUserApi(Number(id))));
					message.success(t("common.delete_success"));
					setSelectedRowKeys([]);
					actionRef.current?.reload();
				} catch (err) {
					message.error(t("common.delete_failure"));
				}
			},
		});
	};
		...
		>
			...
			<Splitter style={{ height: "100%", background: token.colorBgContainer, overflow: "hidden" }}>
				<Splitter.Panel defaultSize="25%" min="15%">
					<ProCard title={t("user.tree_title")} headerBordered styles={{ header: { height: LAYOUT_CONSTANTS.HEADER_HEIGHT } }} extra={
						<Space size={2}>
						...
						{showOrgFilter && (
							<div style={{ padding: "12px", background: token.colorFillAlter, marginBottom: 8, borderRadius: token.borderRadius }}>
						...
						<div style={{ flex: 1, overflowY: "auto" }}>
							<Tree showIcon blockNode treeData={treeData} expandedKeys={expandedKeys} onExpand={setExpandedKeys as any} selectedKeys={[selectedKey]} onSelect={(keys) => { const key = keys.length > 0 ? keys[0] : "root"; setSelectedKey(key); setTimeout(() => actionRef.current?.reload(), 0); }} />
						</div>
					</ProCard>
				</Splitter.Panel>

				<Splitter.Panel>
					<ProCard title={t("user.list_title")} headerBordered styles={{ header: { height: LAYOUT_CONSTANTS.HEADER_HEIGHT } }} extra={
						<Space size={8}>
							<Tooltip title={t("common.search")}>
							...
							<Tooltip title={t("user.new_user")}>
								<Button key="add" icon={<PlusOutlined />} type="primary" size="small" onClick={() => { setEditingUser(null); setDrawerVisible(true); }} />
							</Tooltip>
						</Space>
					}>
							...
							<div style={{ flex: 1, overflow: "hidden" }}>
								<ProTable<User> 
									...
									tableAlertRender={({ selectedRowKeys, onCleanSelected }) => (
										<Space size={24}>
											<span>{t("common.selected_count", { count: selectedRowKeys.length }, `${selectedRowKeys.length}개 선택됨`)}</span>
											<a style={{ marginLeft: 8 }} onClick={onCleanSelected}>
												{t("common.clear_selection", "선택 해제")}
											</a>
										</Space>
									)}
