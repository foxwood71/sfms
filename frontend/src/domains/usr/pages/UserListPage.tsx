...
/**
 * 사용자 목록 관리 페이지 (Refined Single Bento Standard - Fixed Types)
 */
const UserListPage: React.FC = () => {
	...
	// [FIX] React 19: useRef 반드시 초기값 null 필요
	const actionRef = useRef<ActionType>(null);

	const [drawerVisible, setDrawerVisible] = useState(false);
	const [editingUser, setEditingUser] = useState<User | null>(null);
	const [selectedKey, setSelectedKey] = useState<React.Key>("root");
	// [FIX] AntD SizeType은 'large', 'middle', 'small' 임. 'default'는 'large'에 대응.
	...
	const { data: orgResponse, isFetching: isOrgFetching } = useQuery({
		...
		queryFn: () =>
			getOrganizationsApi("tree", showInactiveOrg ? undefined : true),
	});

	// --- [SYNC] 트리 제어 유틸리티 (useCallback으로 린트 해결) ---
	...
	const posMap = useMemo(() => {
		...
		// [FIX] forEach 내에서 값을 반환하지 않도록 중괄호 추가
		posCodes?.forEach((c) => {
	...
	const saveMutation = useMutation({
		mutationFn: (values: UserFormValues) => {
			...
			const payload = {
				...rest,
				role_ids,
				metadata: { ...(editingUser?.metadata || {}), pos, duty },
			};
			if (editingUser)
				return updateUserApi(editingUser.id, payload as UpdateUserParams);
		...
		const mapToTree = (
			...
			return items
				.map((item) => {
					const isMatched =
						!orgSearchValue ||
						item.name.toLowerCase().includes(orgSearchValue.toLowerCase());
					const childrenNodes = item.children
						? mapToTree(item.children, parentMatched || isMatched)
						: [];
					if (!parentMatched && !isMatched && childrenNodes.length === 0)
						return null;
					return {
						key: item.id,
	...
	return (
		...
		>
								...
								<Space size={2}>
									...
									<Button
										...
										icon={
											<FilterOutlined
												style={{
													color: showOrgFilter ? token.colorPrimary : undefined,
												}}
											/>
										}
						...
						>
							{showOrgFilter && (
								<div
									style={{
										padding: "12px 20px",
										background: token.colorFillAlter, // [FIX] colorBgSubtle 대신 colorFillAlter 사용
								...
								>
									...
									>
										<span
											style={{ fontSize: "11px", color: token.colorTextSecondary }}
										>
			...
			<UserFormDrawer
				...
				initialOrgId={selectedKey === "root" ? undefined : Number(selectedKey)}
