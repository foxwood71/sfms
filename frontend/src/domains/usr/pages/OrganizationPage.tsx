...
import { Tree, Button, App, Empty, Spin, Popconfirm, Space, theme, Input, Tooltip, Switch, Splitter, Divider, Row, Col } from "antd";
...
import type { Organization, CreateOrgParams, UpdateOrgParams } from "../types";
...
const OrganizationPage: React.FC = () => {
	...
	const { data: orgResponse, isLoading, isFetching } = useQuery({
	...
	const allKeys = useMemo(() => ["root", ...(orgResponse?.data ? getAllKeys(orgResponse.data) : [])], [orgResponse]);

	useEffect(() => {
		if (searchValue && orgResponse?.data) { setExpandedKeys(allKeys); } 
	...
	const flatData = useMemo(() => {
		const flatten = (items: Organization[]): Organization[] => {
			...
			for (const item of items) {
				result.push(item);
				if (item.children && item.children.length > 0) { result = result.concat(flatten(item.children)); }
			}
			return result;
		};
	...
	useEffect(() => {
		if (searchValue && flatData.length > 0) {
			const match = flatData.find(org => org.name.toLowerCase().includes(searchValue.toLowerCase()));
	...
	const treeData = useMemo(() => {
		const mapToTree = (items: Organization[], parentMatched = false): any[] => {
			return items
				.map((item) => {
					...
					return {
						key: item.id,
						title: item.is_active ? item.name : (
							<span style={{ color: token.colorTextDisabled, textDecoration: "line-through", opacity: 0.6, fontStyle: "italic" }}>
								{item.name} (비활성)
							</span>
						),
		...
		return [{ key: "root", title: "전체 조직도", icon: <TeamOutlined />, children: orgResponse?.data ? mapToTree(orgResponse.data) : [] }];
	...
	const saveMutation = useMutation({
		mutationFn: (values: any) => {
			...
			if (selectedKey) return updateOrganizationApi(selectedKey, payload as UpdateOrgParams);
			throw new Error("대상 조직이 선택되지 않았습니다.");
		},
		onSuccess: async (response) => {
			message.success("조직 정보가 안전하게 저장되었습니다.");
			await queryClient.invalidateQueries({ queryKey: ["organizations"] });
			if (isAdding && response.data?.id) { setSelectedKey(response.data.id); }
			setIsAdding(false); setIsEditing(false);
		},
		onError: (error: any) => { 
			// 백엔드에서 던진 구체적인 메시지가 있다면 노출, 없으면 기본 메시지
	...
	const deleteMutation = useMutation({
		...
		onSuccess: async () => {
			...
			setSelectedKey(null); setIsEditing(false);
			await queryClient.invalidateQueries({ queryKey: ["organizations"] });
		},
		onError: (error: any) => { 
			const errorMsg = error.response?.data?.message || error.message || "삭제 권한이 없거나 하위 데이터가 존재합니다.";
	...
	return (
		...
		>
			<Splitter style={{ height: "100%", background: token.colorBgContainer, borderRadius: token.borderRadiusLG, border: `1px solid ${token.colorBorderSecondary}`, overflow: "hidden" }}>
					...
					<ProCard 
						...
						extra={
							<Space size={4} style={{ height: "32px", display: "flex", alignItems: "center" }}>
								...
								<Tooltip title="검색 (ESC/Tab)"><Button type="text" size="middle" icon={<SearchOutlined style={{ color: showSearch ? token.colorPrimary : undefined }} />} onClick={() => showSearch ? closeSearch() : setShowSearch(true)} /></Tooltip>
						...
						<div style={{ padding: "24px 24px 16px 24px", flexShrink: 0 }}>
							<div style={{ padding: "8px 12px", background: token.colorFillAlter, borderRadius: token.borderRadiusLG, border: `1px solid ${token.colorBorderSecondary}` }}>
								...
								<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
									...
									<Space size={4}>
										<span style={{ fontSize: "11px", color: showInactive ? token.colorError : token.colorSuccess, fontWeight: 500 }}>
											{showInactive ? "비활성 조직 포함" : "활성 조직만"}
										</span>
				...
				<Splitter.Panel>
					<ProCard 
						title={isAdding ? "신규 조직 등록" : (selectedOrg ? `[${selectedOrg.name}] 상세 정보` : "조직 상세 정보")} 
						headerBordered
						headerStyle={{ minHeight: "56px" }}
						extra={
							<div style={{ height: "32px", display: "flex", alignItems: "center" }}>
								{!isAdding && selectedOrg && (
									<Space size={8}>
										...
										<Popconfirm title="삭제?" onConfirm={() => deleteMutation.mutate(selectedOrg.id)} okText="삭제" cancelText="취소">
											<Button danger type="text" icon={<DeleteOutlined />} loading={deleteMutation.isPending} size="small">삭제</Button>
										</Popconfirm>
									</Space>
								)}
							</div>
						...
						{(selectedOrg || isAdding) ? (
							<ProForm
								...
								submitter={isEditing ? {
									render: (props) => (
										<div style={{ 
											position: "sticky", bottom: -24, left: -24, right: -24, padding: "16px 24px", 
											background: token.colorBgContainer, borderTop: `1px solid ${token.colorBorderSecondary}`,
											zIndex: 10, display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "auto"
										}}>
											<Button icon={<CloseOutlined />} onClick={handleCancel}>취소</Button>
											<Button type="primary" icon={<SaveOutlined />} onClick={() => props.form?.submit()} loading={saveMutation.isPending}>
												{isAdding ? "등록" : "저장"}
											</Button>
										</div>
									),
							...
							>
								<Row gutter={24}>
									<Col span={12}>
										<ProFormText name="code" label="조직 코드" rules={[{ required: true }, { max: 50 }]} disabled={!isAdding || isDisabled} getValueFromEvent={(e) => e.target.value.toUpperCase()} fieldProps={{ showCount: true, maxLength: 50, style: { textTransform: "uppercase" } }} />
									</Col>
									<Col span={12}>
										<ProFormText name="name" label="조직 명칭" rules={[{ required: true, message: "명칭 필수" }, { max: 100 }]} disabled={isDisabled} fieldProps={{ showCount: true, maxLength: 100 }} />
									</Col>
								</Row>

								<ProForm.Item name="parent_id" label="상위 조직">
									<OrgTreeSelect placeholder="상위 선택" activeOnly={false} disabled={isDisabled} />
								</ProForm.Item>

								<Row gutter={24}>
									<Col span={12}>
										<ProFormDigit name="sort_order" label="정렬 순서" initialValue={10} min={0} max={9999} disabled={isDisabled} />
									</Col>
									<Col span={12}>
										<ProFormSwitch name="is_active" label="조직 상태" disabled={isDisabled} fieldProps={{ checkedChildren: "활성", unCheckedChildren: "비활성" }} />
									</Col>
								</Row>
								...
								{!isAdding && selectedOrg && (
									<>
										<Divider orientation="left" style={{ margin: "16px 0" }}>
											<Space><HistoryOutlined /> 이력 정보</Space>
										</Divider>
										<Row gutter={24}>
											<Col span={12}>
												<ProFormDateTimePicker name="created_at" label="생성 일시" disabled fieldProps={{ style: { width: "100%" } }} />
											</Col>
											<Col span={12}>
												<ProFormDateTimePicker name="updated_at" label="최종 수정 일시" disabled fieldProps={{ style: { width: "100%" } }} />
											</Col>
										</Row>
									</>
								)}
