    ...
    return menus
        .map((item) => {
            ...
            if (item.resource === "ALL" || !item.resource) {
                const newItem = { ...item };
                if (newItem.children) {
                    const filtered = filterMenus(newItem.children, permissions, superUser);
            ...
            const isAllowed = !!userActions && (userActions.includes("*") || userActions.includes(requiredAction));

            if (!isAllowed) return null;

            const newItem = { ...item };
            if (newItem.children) {
                const filteredChildren = filterMenus(newItem.children, permissions, superUser);
